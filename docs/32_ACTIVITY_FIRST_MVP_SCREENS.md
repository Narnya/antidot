...
| **T6 ✅** | **Settings** | Settings screen (account/email, «Мой профиль»), blocked-users list + unblock, sign out, and hard **delete account** via a security-definer `delete_own_account()` RPC (cascades all data; no service role on client — Инв. 12); entry links from home + profile self-view | account hygiene — **built; typecheck green; migration applied + verified live (delete cascade + owner survives; unblock self-only RLS)** |
